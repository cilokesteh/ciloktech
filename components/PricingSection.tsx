...
export default function PricingSection() {
  return (
    <section className="py-20 md:py-28 bg-[#f6f6f5] border-y border-gray-200 px-6" id="harga">
      <div className="max-w-6xl mx-auto">
        <div className="max-w-2xl mx-auto text-center mb-14">
          <div className="inline-flex items-center gap-2 text-[11px] font-bold tracking-[0.15em] uppercase text-gray-900 bg-white border border-gray-200 px-3 py-1 rounded-full mb-5 shadow-sm">
            Harga transparan, tanpa hidden fee
          </div>
          <h2 className="text-[32px] md:text-[46px] font-extrabold tracking-[-0.03em] leading-[0.9] text-gray-900">
            Bayar sesuai value, <br />
            <span className="text-gray-400">bukan janji manis.</span>
          </h2>
          ...
          {plans.map((plan, idx) => (
            <div
              ...
              className={`relative rounded-[22px] p-7 flex flex-col ${
                plan.featured
                  ? "bg-gray-900 text-white shadow-[0_24px_64px_rgba(0,0,0,0.22)] md:-mt-4 md:mb-4 border border-gray-800"
                  : "bg-white border border-gray-200 hover:border-gray-900 transition-colors"
              }`}
            >
              {plan.featured && (
                <div className="absolute -top-3 left-7 inline-flex items-center gap-1.5 bg-cyan-400 text-gray-900 text-[10px] font-extrabold tracking-widest uppercase px-3 py-1 rounded-full">
                  <span className="w-1.5 h-1.5 rounded-full bg-gray-900 animate-pulse" /> POPULER
                </div>
              )}

              <div className="mb-6">
                <h3 className={`text-[18px] font-bold tracking-tight ${plan.featured ? "text-white" : "text-gray-900"}`}>
                  {plan.name}
                </h3>
                ...
                <div className={`text-[12px] mt-1 ${plan.featured ? "text-gray-400" : "text-gray-500"}`}>{plan.unit}</div>
                <p className={`text-[13px] leading-relaxed mt-4 ${plan.featured ? "text-gray-400" : "text-gray-600"}`}>{plan.desc}</p>
              </div>
                  ...
                  <li key={i} className="flex gap-2.5 text-[13px] leading-snug">
                    <span
                      className={`mt-0.5 w-5 h-5 rounded-full flex items-center justify-center text-[11px] shrink-0 ${
                        plan.featured ? "bg-white/10 text-white" : "bg-gray-900 text-white"
                      }`}
                    >
                      ✓
                    </span>
                    <span className={plan.featured ? "text-gray-200" : "text-gray-700"}>{f}</span>
                  </li>
                ))}
              </ul>
              ...
              {plan.featured && (
                <div className="text-[11px] text-center text-gray-500 mt-3">Balas &lt;2 jam • rata-rata live 3 hari</div>
              )}
            </div>
          ))}
        </div>

        <div className="mt-10 text-center text-[12.5px] text-gray-500">
          Butuh paket lain? <a href="https://t.me/ciloktech" className="font-bold text-gray-900 underline underline-offset-4">Chat dulu</a> — gue bikinin custom sesuai budget.
        </div>
      </div>
    </section>
  );
