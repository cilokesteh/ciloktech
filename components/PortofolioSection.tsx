    ...
    <section className="py-20 md:py-28 bg-white dark:bg-[#0a0a0a] px-6 transition-colors duration-300" id="portofolio">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {projects.map((p, i) => (
            <div key={i} className="group rounded-[20px] border border-gray-200 dark:border-white/10 bg-white dark:bg-[#171717] overflow-hidden hover:border-gray-900 dark:hover:border-white hover:shadow-[0_20px_50px_rgba(0,0,0,0.08)] dark:hover:shadow-[0_20px_50px_rgba(0,0,0,0.4)] transition-all duration-300 flex flex-col">
              <div className={`h-[160px] bg-gradient-to-br ${accents[i] || accents[0]} relative p-5 flex flex-col justify-between overflow-hidden`}>
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-bold tracking-wide uppercase bg-white/90 backdrop-blur px-2.5 py-1 rounded-full text-gray-900">{p.cat}</span>
                  <span className="text-[11px] font-bold bg-black/20 backdrop-blur text-white px-2.5 py-1 rounded-full">{p.stats}</span>
                </div>
                <div className="mt-auto"><div className="inline-flex h-px w-10 bg-white/60 mb-3" /><div className="text-white font-extrabold text-[18px] leading-tight drop-shadow-sm">{p.title}</div></div>
              ...
              <div className="p-5 flex flex-col flex-1">
                ...
                <div className="flex flex-wrap gap-1.5 mt-auto">
                  {(TAGS[i] || TAGS[0]).map((tag, j) => (
                    <span key={j} className="text-[11px] font-medium bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-gray-700 dark:text-gray-300 px-2.5 py-1 rounded-full group-hover:bg-gray-900 dark:group-hover:bg-white group-hover:text-white dark:group-hover:text-black group-hover:border-gray-900 dark:group-hover:border-white transition">{tag}</span>
                  ))}
                </div>
              </div>
            </div>
