    ...
    <main className="pt-16 bg-white dark:bg-[#0a0a0a] min-h-screen">
      <div className="max-w-4xl mx-auto px-6 py-12 md:py-20">
        <div className="flex items-start justify-between gap-6">
          <div>
            ...
            <h1 className="text-[34px] md:text-[56px] font-extrabold tracking-[-0.03em] leading-[0.9] text-gray-900 dark:text-white">
              Changelog
              <span className="block text-gray-400 dark:text-gray-500 text-[22px] md:text-[28px] mt-2 font-bold tracking-tight">{isId ? "Minggu demi minggu — transparan dan terukur." : "Week by week — no fluff, just progress."}</span>
            </h1>
            <p className="text-[15px] leading-relaxed text-gray-600 dark:text-gray-400 mt-6 max-w-[600px]">
              {isId ? "Saya membangun CilokTech one-man studio secara transparan. Setiap commit, artikel blog, favicon, hingga kalkulator — tercatat di sini. Transparan, agar Anda tahu studio ini aktif dan bukan template yang sudah mati." : "Building CilokTech one-man studio in public. Every commit, blog post, favicon, and calculator — documented here. Fully transparent, so you can see this studio is actively maintained."}
            </p>
          </div>
        ...
        <div className="mt-16 rounded-[20px] bg-gray-900 dark:bg-white text-white dark:text-black p-7 flex flex-col md:flex-row items-center justify-between gap-5">
          <div>
            <div className="font-extrabold text-[18px]">{isId ? "Ingin melihat repositori & progress secara langsung?" : "Want to see the repository and live progress?"}</div>
            <div className="text-[13px] opacity-70 mt-1">{isId ? "Semua build, commit, dan desain tersedia di GitHub. One-man studio — terbuka untuk umum." : "All builds, commits, and designs on GitHub. One-man studio — open source mindset."}</div>
          </div>
